...
Az alábbi C# példa bemutatja, hogyan toomanage Azure idő adatsorozat Insights környezet adatokra hivatkoztak.
Futó hello minta előtt győződjön meg arról, hello lépések elvégzése.
1. A referencia-adatkészlet használatával lett létrehozva [Ez a cikk](time-series-insights-add-reference-data-set.md).
2. hello hozzáférési jogkivonat hello alkalmazást futtató rendszer beszerzett hello Azure Active Directory API használatos. Ez a token kell átadni hello `Authorization` minden lekérdezés API-kérelem fejlécében. Nem interaktív alkalmazások beállítása, lásd: hello [hitelesítési és engedélyezési](time-series-insights-authentication-and-authorization.md) cikk.
3. Minden hello konstans definiálva hello minta hello elején megfelelően van beállítva.
