...
Még nem rendelkezik egy csomagot? Próbálja meg a [málna Pi 3 emulátor](https://blogs.msdn.microsoft.com/iliast/2016/11/10/how-to-emulate-raspberry-pi/). Vagy egy új csomag vásárlása [Itt](https://azure.microsoft.com/develop/iot/starter-kits).
...
1. Töltse le a Raspbian.
   1. [Töltse le a Pixel Raspbian Jessie](https://www.raspberrypi.org/downloads/raspbian/) (a .zip-fájlt).
...
Használják a breadboard és átkötés LED és egy BME280 Pi kell kapcsolódniuk az alábbiak szerint. Ha még nem rendelkezik az érzékelő, hagyja ki ezt a szakaszt.
...
1. A számítógép a következő SSH-ügyfél segítségével a málna Pi csatlakozni.
    - [A puTTY](http://www.putty.org/) Windows. A Pi SSH-kapcsolaton keresztül csatlakozzanak IP-címe van szüksége.
    - A beépített SSH-ügyfél Ubuntu vagy macOS. Előfordulhat, hogy kell futtatnia `ssh pi@<ip address of pi>` Pi SSH-kapcsolaton keresztül csatlakozni.
...
1. A Pi Node.js és NPM telepíthető.
   ...
   Először ellenőrizze a Node.js-verzió a következő paranccsal. 
   
   ```bash
   node -v
   ```

   Ha verziószáma kisebb, mint a 4.x-es, vagy nincs nem Node.js a Pi, majd futtassa a következő parancs futtatásával telepítse vagy frissítse a Node.js.
...
1. A következő parancsot az összes csomag telepítéséhez. Ez magában foglalja az Azure IoT-eszközök SDK, BME280 érzékelő és huzalozási Pi függvénytárak.
   ...
   ```
   > [!NOTE] 
   A hálózati kapcsolatban a telepítési folyamat denpening több percig is eltarthat.
...
1. Nyissa meg a konfigurációs fájlban a következő parancsok futtatásával:
   ...
   Két elem ebben a fájlban configurate is. Az első egy `interval`, amely megadja, hogy két felhőbe küldött üzeneteket közötti időközt. A második érték `simulatedData`, vagyis az, hogy szimulált érzékelőadatait vagy nem logikai értéket.
