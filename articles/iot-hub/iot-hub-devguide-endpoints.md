...
* **Eszköz Identitáskezelés**. Minden egyes IoT-központ tesz elérhetővé, HTTP REST-végpontok eszköz identitások kezeléséhez (létrehozása, beolvasása, frissítése és törlése). [Eszköz identitások] [ lnk-device-identities] eszköz hitelesítési és hozzáférés-vezérlés használhatók.
* **A két kezelés**. Minden egyes IoT-központ tesz elérhetővé, HTTP REST-végpont szolgáltatás irányuló lekérdezés és a frissítés [eszköz twins] [ lnk-twins] (címkék és a tulajdonságok frissítése).
* **Felügyeleti feladatok**. Minden egyes IoT-központ tesz elérhetővé, lekérdezését és kezelését szolgáltatás irányuló HTTP REST-végpont [feladatok][lnk-jobs].
* **Eszköz végpontok**. Az egyes eszközök a identitás beállításjegyzék az IoT-központ tesz elérhetővé, végpontok:
  ...
  * *Közvetlen módszer kérések fogadásához*. Egy eszköz használja ehhez a végponthoz tartozó [közvetlen módszer][lnk-methods]a kérelmeket.
    ...
    Az eszköz twins és módszerek végpontok érhetők el, csak akkor, ha használja a [MQTT v3.1.1] [ lnk-mqtt] protokoll.

* **Szolgáltatás végpontjait**. Minden egyes IoT-központ a megoldás háttérrendszeréhez kommunikálnak az eszközök végpontjainak tesz elérhetővé. Egy kivétellel ezeket a végpontokat csak akkor jelennek meg használatával a [AMQP] [ lnk-amqp] protokoll. A metódus meghívása végpont a HTTP protokollon keresztül kommunikál.
