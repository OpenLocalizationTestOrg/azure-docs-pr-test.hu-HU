...
---
# <a name="azure-ad-b2c-use-the-graph-api"></a>Az Azure AD B2C: Használja a Graph API
...
> [!IMPORTANT]
> A Graph API használata a B2C bérlőre, szüksége lesz egy dedikált alkalmazás regisztrálása az általános *App regisztrációk* panel az Azure portálon **nem** az Azure AD B2C *alkalmazások* panelen. Nem használhat újra a már meglévő B2C alkalmazások az Azure AD B2C regisztrált *alkalmazások* panelen.
...
6. A beállítások panelen kattintson a **kulcsok** , és adja hozzá egy új kulcsot (más néven ügyfélkulcs). Is másolja azt egy későbbi szakasz ismerteti a használatra.
...
1. A Folytatás az Azure-portál alkalmazás regisztrációk panelen válassza ki az alkalmazását.
2. A beállítások panelen kattintson a **szükséges engedélyek**.
3. A szükséges engedélyekkel a panelen, kattintson a **Windows Azure Active Directory**.
4. A hozzáférés engedélyezése a panelen, jelölje ki a **címtáradatok olvasása és írása** engedélyt **Alkalmazásengedélyek** kattintson **mentése**.
5. Végezetül vissza a szükséges engedélyek panelen kattintson a a **engedélyt adjon** gombra.
...
Első lépésként töltse le és telepítse a [Microsoft Online Services bejelentkezési segéd](http://go.microsoft.com/fwlink/?LinkID=286152). Ezután töltse le és telepítse a [64 bites Azure Active Directory-modul Windows PowerShell](http://go.microsoft.com/fwlink/p/?linkid=236297).

A PowerShell-modul telepítése után nyissa meg a Powershellt, és csatlakozzon a B2C-bérlő. Futtatása után `Get-Credential`, is meg kell adnia egy felhasználónevet és jelszót, írja be a felhasználónevet és a B2C bérlő rendszergazdai fiók jelszavát.
