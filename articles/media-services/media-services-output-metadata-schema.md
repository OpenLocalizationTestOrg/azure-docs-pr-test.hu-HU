...
A kódolási feladat vagy társítva egy bemeneti eszköz (eszközök) kívánja tooperform egyes kódolási feladatokhoz. Például kódolása egy MP4 fájl tooH.264 MP4 adaptív sávszélességű beállítja; Hozzon létre egy miniatűrre. átfedések létrehozása. Létrehozása után egy feladatot kimeneti eszközként hozott létre.  hello kimeneti adategységen videót, hangot, miniatűrök, a stb hello kimeneti adategységen is hello kimeneti adategységen vonatkozó metaadatok fájlt tartalmaz. hello hello metaadatok XML-fájl neve van hello a következő formátumban: &lt;source_file_name&gt;_manifest.xml (például BigBuckBunny_manifest.xml).  

Ha azt szeretné, hogy tooexamine hello metaadatfájl, létrehozhat egy **SAS** lokátor és a letöltési hello fájl tooyour helyi számítógépen.  

Ez a témakör ismerteti a mely hello kimeneti metada hello elemek és hello XML-séma típusú (&lt;source_file_name&gt;_manifest.xml) alapul. Hello bemeneti eszköz metaadatait tartalmazó fájl hello kapcsolatos információkért lásd: [bemeneti metaadatok](media-services-input-metadata-schema.md).  

> [!NOTE]
> Hello teljes séma kódot és az XML-példa hello Ez a témakör végén található.  
>
>

## <a name="AssetFiles "></a>AssetFiles gyökérelem
Hello kódolási feladat AssetFile bejegyzések gyűjteményét.  

### <a name="child-elements"></a>Gyermekelemek
| Név | Leírás |
| --- | --- |
| **AssetFile**<br/><br/> minOccurs = "0" maxOccurs = "1" |Egy [AssetFile elem](media-services-output-metadata-schema.md) részét képező hello AssetFiles gyűjtemény. |
...
| Név | Típus | Leírás |
| --- | --- | --- |
| **Name (Név)**<br/><br/> Szükséges |**xs:String** |hello media eszköz fájl neve. |
| **Méret**<br/><br/> minInclusive = "0"<br/><br/> Szükséges |**xs:Long** |Mérete bájtban hello objektumfájlt. |
| **Időtartam**<br/><br/> Szükséges |**DURATION típusú** |Tartalom play hátsó időtartama. |

### <a name="child-elements"></a>Gyermekelemek
| Név | Leírás |
| --- | --- |
| **Adatforrások** |Feldolgozott gyűjtemény bemeneti/media forrásfájljainak, a rendezés tooproduce a AssetFile. További információkért lásd: [Forráselem](media-services-output-metadata-schema.md). |
| **VideoTracks**<br/><br/> minOccurs = "0" maxOccurs = "1" |Minden egyes fizikai AssetFile nulla vagy több videó nyomon követi a megfelelő tárolót formátumra időosztásos azt is tartalmazhat. Ez az adott videó nyomon követi hello gyűjteménye. További információkért lásd: [VideoTracks elem](media-services-output-metadata-schema.md). |
| **AudioTracks**<br/><br/> minOccurs = "0" maxOccurs = "1" |Minden fizikai AssetFile azt egy megfelelő tárolót formátumra időosztásos nulla vagy több zeneszámok tartalmazhat. Ez az összes adott zeneszámok hello gyűjteménye. További információkért lásd: [AudioTracks elem](media-services-output-metadata-schema.md). |

## <a name="Sources "></a>Adatforrások elem
Feldolgozott gyűjtemény bemeneti/media forrásfájljainak, a rendezés tooproduce a AssetFile.  
...
Minden egyes fizikai AssetFile nulla vagy több videó nyomon követi a megfelelő tárolót formátumra időosztásos azt is tartalmazhat. Ez az adott videó nyomon követi hello gyűjteménye.  
...
| Név | Leírás |
| --- | --- |
| **VideoTrack**<br/><br/> minOccurs = "1" maxOccurs = "unbounded" |Egy adott videó hello szülő AssetFile nyomon. További információkért lásd: [VideoTrack elem](media-services-output-metadata-schema.md#VideoTrack). |

## <a name="VideoTrack"></a>VideoTrack elem
Egy adott videó hello szülő AssetFile nyomon.  
...
| Név | Típus | Leírás |
| --- | --- | --- |
| **Azonosítója**<br/><br/> minInclusive = "0"<br/><br/> Szükséges |**xs:INT** |Ez a videó track nulla alapú indexét. **Megjegyzés:** ez nem szükségszerűen hello TrackID használt MP4-fájlokat. |
| **FourCC**<br/><br/> Szükséges |**xs:String** |Videó kodek FourCC kódot. |
| **Profil** |**xs:String** |H264 profil (csak a megfelelő tooH264 kodek). |
| **Szint** |**xs:String** |H264 szint (csak a megfelelő tooH264 kodek). |
...
| **Átviteli sebesség**<br/><br/> minInclusive = "0"<br/><br/> Szükséges |**xs:INT** |Kilobit / másodperc, a hello AssetFile kiszámított videó átlagos átviteli sebességet. Csak a hello elemi adatfolyam tartalmat megjeleníti, és nem tartalmaz hello csomagolás terhelés. |
| **TargetBitrate**<br/><br/> minInclusive = "0"<br/><br/> Szükséges |**xs:INT** |A videó nyomon követése, az átlagos sávszélességű TARGET kért keresztül hello kódolási beállításkészlet a kilobit / másodperc. |
...
Minden fizikai AssetFile azt egy megfelelő tárolót formátumra időosztásos nulla vagy több zeneszámok tartalmazhat. Ez az összes adott zeneszámok hello gyűjteménye.  
...
| Név | Leírás |
| --- | --- |
| **AudioTrack**<br/><br/> minOccurs = "1" maxOccurs = "unbounded" |Egy adott hang hello szülő AssetFile nyomon. További információkért lásd: [AudioTrack elem](media-services-output-metadata-schema.md). |

## <a name="AudioTrack "></a>AudioTrack elem
Egy adott hang hello szülő AssetFile nyomon.  
...
| Név | Típus | Leírás |
| --- | --- | --- |
| **Azonosítója**<br/><br/> minInclusive = "0"<br/><br/> Szükséges |**xs:INT** |A hang követése nulla alapú indexét. **Megjegyzés:** ez nem szükségszerűen hello TrackID használt MP4-fájlokat. |
...
| **Átviteli sebesség**<br/><br/> minInclusive = "0"<br/><br/> Szükséges |**xs:INT** |Bit / másodperc, a hello AssetFile kiszámított átlagos átviteli sebességet. Csak a hello elemi adatfolyam tartalmat megjeleníti, és nem tartalmaz hello csomagolás terhelés. |
| **A BitsPerSample**<br/><br/> minInclusive = "0"<br/><br/> Szükséges |**xs:INT** |Bit / minta hello wFormatTag formátumban írja be. |
...
| **IntegratedLoudnessSpeechPercentage**<br/><br/> minInclusive = "0" maxInclusive "100" = |**xs:float** |Beszéd tartalom hello program százalékos keresztül. |
...
## <a name="xml"></a>XML-példa
 hello hello kimeneti metaadatait tartalmazó fájl egy példa látható.  
