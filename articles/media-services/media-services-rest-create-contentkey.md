...
A Media Services lehetővé teszi új toocreate, és titkosított eszközök. A **ContentKey** biztosít a biztonságos hozzáférést tooyour **eszköz**s. 

Amikor létrehoz egy új eszközt (például előtt [fájlok feltöltése](media-services-rest-upload-files.md)), megadhatja a titkosítási beállítások a következő hello: **StorageEncrypted**, **CommonEncryptionProtected**, vagy **EnvelopeEncryptionProtected**. 

Ha eszközök tooyour ügyfelek kézbesíti, akkor [dinamikusan titkosított eszközök toobe konfigurálása](media-services-rest-configure-asset-delivery-policy.md) hello két titkosítások használatára a következő egyike: **DynamicEnvelopeEncryption** vagy  **DynamicCommonEncryption**.
...
3. Hello nyilvános kulcsával hello X.509 tanúsítvány a tartalom kulcs titkosításához. 
   ...
   Media Services .NET SDK RSA végrehajtásakor hello titkosítási OAEP típusú használ.  Látható egy példa a hello [EncryptSymmetricKeyData függvény](https://github.com/Azure/azure-sdk-for-media-services/blob/dev/src/net/Client/Common/Common.FileEncryption/EncryptionUtils.cs).
4. Hozzon létre egy hello kulcsazonosító és tartalomkulcsot számítja (hello PlayReady AES-kulcs ellenőrzőösszeg algoritmus alapján) ellenőrzőösszeg értéket. További információkért lásd: hello hello PlayReady fejléc objektum dokumentum "PlayReady AES ellenőrzőösszeg kulcsalgoritmus" szakaszában található [Itt](http://www.microsoft.com/playready/documents/).
   
   hello az alábbiakban látható egy .NET típusú példát, amely kiszámítja a hello kulcsazonosító hello GUID része hello ellenőrzőösszeg és hello törölje tartalomkulcsot.
...
5. Hozzon létre hello tartalomkulcsot hello **EncryptedContentKey** (átalakítás toobase64 kódolású karakterlánc) **ProtectionKeyId**, **ProtectionKeyType**,  **ContentKeyType**, és **ellenőrzőösszeg** értékeket az előző lépésben kapott.
6. Rendeljen hozzá hello **ContentKey** entitás a **eszköz** entitás hello $links művelet keresztül.

Vegye figyelembe, hogy ez a témakör nem szerepelnek toogenerate AES-kulccsal, hogyan hello kulcs titkosításához, és hello ellenőrzőösszeg számítása. 
...
## <a name="create-hello-contentkey"></a>Hello ContentKey létrehozása
...
Hello tartalom hello típus kulcs létrehozása, hogy kell-e állítva mikor hello értékek egyike. Hello a következő értékek közül választhat.
