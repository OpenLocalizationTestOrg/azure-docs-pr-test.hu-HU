...
---
# <a name="prevent-unexpected-costs-with-azure-billing-and-cost-management"></a>Azure számlázás és költség felügyeleti váratlan költségek megakadályozása

Amikor regisztrál az Azure-ba, számos módon teheti tooget segítenek meghatározni, hogy a ráfordítás. A hello [Azure-portálon](https://portal.azure.com/#blade/Microsoft_Azure_Billing/SubscriptionsBlade)hello előfizetés kiválasztásakor, tekintse meg az aktuális költség részletes információkat, és Írás gyakorisága. Emellett [töltse le a korábbi számlákat és fájlok részletességi](billing-download-azure-invoice-daily-usage-date.md). Ha másik projektek vagy csoportok használt erőforrások toogroup költségek, tekintse meg [erőforrás címkézés](../azure-resource-manager/resource-group-using-tags.md). Ha a szervezete jelentési rendszer, hogy szeretné-e toouse, tekintse meg hello [API-k számlázási](billing-usage-rate-card-overview.md). 

A napi használatával kapcsolatos további információkért lásd: [a számlázási megérteni a Microsoft Azure](billing-understand-your-bill.md).
...
Ha az előfizetés egy ingyenes próbaverzióra [Visual Studio](https://azure.microsoft.com/pricing/member-offers/msdn-benefits-details/), nyissa meg Nyomtatókategóriák vagy BizSpark, az Azure majd megismerése [költségkeretek](#spending-limit) tooavoid, hogy az előfizetés unexpectantly le van tiltva. 

## <a name="day-0-before-you-add-azure-services"></a>Nap 0: Előtt hozzá Azure-szolgáltatások
...
Tekintse meg a hello [árképzési Számológép](https://azure.microsoft.com/pricing/calculator/) és [összköltsége tulajdonjoga Számológép](https://aka.ms/azure-tco-calculator) tooget egy becslés hello havi költségét kíváncsiak vagyunk hello szolgáltatást. Például egy A1 Windows virtuális gép (VM) a számítási órák $66.96 USD/hónap ha hagyja hello teljes futásidő becsült toocost:
...
További információkért lásd: [árképzési GYIK](https://azure.microsoft.com/pricing/faq/). Vagy ha azt szeretné, hogy tootalk tooa személy, 1-800-867-1389 hívja.
...
### <a name="understand-limits-and-quotas-for-your-subscription"></a>Korlátok és az előfizetés kvótái

Nincsenek alapértelmezett korlátozások tooeach előfizetés többek között a hello több processzormag és IP-címeket. Kell szem előtt tartva a működés felső korlátjának. További információkért lásd: [Azure-előfizetés és szolgáltatási korlátok, kvóták és megkötések](../azure-subscription-service-limits.md). Egy tooyour korlát növelését vagy a kvóta által kérhet [lépjen kapcsolatba az ügyfélszolgálattal](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade).

## <a name="day-1-as-you-add-services"></a>1. napja: Szolgáltatások hozzáadása

### <a name="review-hello-estimated-cost-in-hello-portal"></a>Felülvizsgálati hello becsült költség hello portálon

Általában amikor hello Azure-portálon ad hozzá egy szolgáltatás, a rendszer havonta egy hasonló becsült költségét jeleníti meg a nézet. Például ha úgy dönt, hogy a Windows virtuális gép méretét hello látja hello becsült havi költségét hello számítási órák:
...
## <a name="cost-reporting"></a>Naponta 2 +: Után szolgáltatások használati megtekintése
...
1. A Microsoft hello [előfizetések panel az Azure-portálon](https://portal.azure.com/#blade/Microsoft_Azure_Billing/SubscriptionsBlade).

2. Válassza ki a kívánt toosee előfizetést. Csak előfordulhat, hogy egy tooselect.

3. Hello költség részletes információkat lásd: kell, és sebessége írása hello előugró panelen. Azt nem támogatja az előfizetéshez (figyelmeztetés fog megjelenni hello felső mellett). Várjon 24 órát, a szolgáltatás hello adatok toopopulate hozzáadása után.
...
4. Érdemes lehet toopin hello nézet tooyour irányítópult.

    ![Képernyőkép a rögzítési toohello irányítópult nézet](./media/billing-getting-started/pin.PNG)

5. Kattintson a **elemzés** hello lista toohello bal oldali toosee hello költség lebontásban erőforrás.

    ![Képernyőfelvétel a hello költség elemző nézet Azure-portálon](./media/billing-getting-started/cost-analysis.PNG)

6. Például különböző tulajdonságok szerint szűrheti [címkék](#tags), erőforráscsoport és timespan. Kattintson a **alkalmaz** tooconfirm hello szűrők és **letöltése** tooexport hello nézet tooa Comma-Separated értékeket tartalmazó (.csv) fájlt.

7. Kattintson egy erőforrás toosee fordítják az előzmények és mennyi azt lett költségszámítás meg minden nap.
...
#### <a name="view-costs-for-all-your-subscriptions-in-hello-billing-blade"></a>Hello számlázási panelen az előfizetések nézet költségei

Ha több előfizetéssel, fiókadminisztrátor hello kezelni, megtekintheti hello összesített számlázási összeg és részletes információkat az előfizetéseket a hello [számlázási panel](https://portal.azure.com/#blade/Microsoft_Azure_Billing/BillingBlade). 
...
### <a name="invoice-and-usage"></a>Az első számlázási időszak után a számla és részletes használati beolvasása

Az első számlázási időszak után letöltheti a Portable Document Format (.pdf) számla és a használat részleteiről Comma-Separated értékeket tartalmazó (.csv). Azt is kérheti a toohave a számla tooyou e-mailben. Ezek a fájlok súgó toounderstand végső soron számlázott tooyou Újdonságok adó, kedvezményeket és kreditek után. Ha tooyour előfizetés fizetési csatolt metódus nem rendelkezik, ezeket a fájlokat nem érhető el, meg lehet. További információkért lásd: [hogyan tooget az Azure számlázás számla és a napi használati adatok](billing-download-azure-invoice-daily-usage-date.md) és [a számlázási megérteni a Microsoft Azure](billing-understand-your-bill.md).

![A .pdf számla képernyőképe](./media/billing-getting-started/invoice.png)

korábban beállított hello címkék hello részletes .csv fájlok jelenik meg:
...
## <a name="other-offers"></a>További erőforrásokat a EA CSP és szponzorálás
