...
> [!IMPORTANT]
> Nem kell egy meglévő HDInsight-fürtre. A jelen dokumentumban leírt lépések hozza létre a következőket:
> 
...
A topológia olvassa be az adatokat az Event Hubs használatával a [org.apache.storm.eventhubs.spout.EventHubSpout](http://storm.apache.org/releases/0.10.1/javadocs/org/apache/storm/eventhubs/spout/class-use/EventHubSpout.html) osztály és HBase használatával írt adatok a [org.apache.storm.hbase.bolt.HBaseBolt](https://storm.apache.org/releases/1.0.1/javadocs/org/apache/storm/hbase/bolt/HBaseBolt.html) osztály. A webhely kommunikál végezhető el [socket.io-client.java](https://github.com/nkzawa/socket.io-client.java).
...
Ez a példa használata előtt létre kell hoznia egy Azure Event hubs Eseményközpontot, amelyben a Storm-topológia a.

### <a name="configure-event-hub"></a>Az Event Hubs konfigurálása

Az Event Hubs ebben a példában az adatforrást. Az alábbi lépések segítségével létrehoz egy Eseményközpontot.

1. Az a [Azure-portálon](https://portal.azure.com), jelölje be **+ új** -> **az eszközök internetes hálózatát** -> **Event Hubs**.
2. Az a **létrehozása Namespace** területen a következő feladatokat:
   
   1. Adjon meg egy **neve** a névtérhez.
   2. Válasszon tarifacsomagot. **Alapszintű** elegendő-e ebben a példában.
   3. Válassza ki az Azure **előfizetés** használatára.
   4. Válasszon ki egy meglévő erőforráscsoportot, vagy hozzon létre egy újat.
   5. Válassza ki a **hely** az Event Hubs számára.
   6. Válassza ki **rögzítés az irányítópulton**, és kattintson a **létrehozása**.

3. A létrehozási folyamat befejezését követően a névtérhez az Event Hubs információk jelennek meg. Itt válassza **+ Hozzáadás Eseményközpont**. Az a **Eseményközpont létrehozása** területen adjon meg egy **sensordata**, majd válassza ki **létrehozása**. Az alapértelmezett értékeket, a többi mezőt hagyja.
4. Az Event Hubs nézetből a névtérhez, válassza ki a **Event Hubs**. Válassza ki a **sensordata** bejegyzés.
5. Az Event Hubs sensordata jelölje ki **megosztott elérési házirendek**. Használja a **+ Hozzáadás** szabályzatok beállítása a következő hivatkozásra:
...
1. Válassza ki mindkét házirend, és jegyezze fel a **elsődleges kulcs** érték. Az érték a későbbi lépésekben mindkét házirend van szükség.
...
> [!WARNING]
> A Windows környezet használatakor jelenhet meg a `java.io.IOException` a jelentés futtatásakor a topológia helyileg. Ha igen, helyezze át HDInsight a topológia futó.
...
2. Nyissa meg a `app.js` fájlt egy szövegszerkesztőben, és adja meg a korábban beszerzett Eseményközpont információkat:
   ...
   > [!NOTE]
   > Ebben a példában feltételezzük, hogy használt `sensordata` az Eseményközpont neveként. És hogy `devices` , amelyen a házirend nevét, egy `Send` jogcímek.
...
A HDInsight HBase kapcsolatos további információkért tekintse meg a [HBase a HDInsight áttekintése](hdinsight-hbase-overview.md).
