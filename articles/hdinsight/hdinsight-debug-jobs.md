...
Amikor HDInsight fürtöt hoz létre, a 6 táblázatok automatikusan jönnek létre Linux-alapú fürtökhöz az alapértelmezett Table storage-ban:
...
* hadoopinstalllog: naplózási események/kivételek észlelt, amikor a fürt Hadoop telepítését. Ez a táblázat hibakeresés egyéni paraméterekkel létrehozott fürtök kapcsolatos problémák hasznos lehet.
* hadoopservicelog: összes Hadoop-szolgáltatás által rögzített események/kivételek naplóját. Ez a táblázat-hibakeresés a HDInsight-fürtökön feladatleállások kapcsolatos problémák hasznos lehet.

A tábla fájlnevek **u<ClusterName>DDMonYYYYatHHMMSSsss<TableName>**.

Ezek a táblázatok a következő mezőket tartalmazza:
...
A Power Query telepíthető [www.microsoft.com/en-us/download/details.aspx?id=39379](http://www.microsoft.com/en-us/download/details.aspx?id=39379). A letöltési lapra a rendszerkövetelményeket lásd:

**Power Query segítségével nyissa meg és elemezheti a napló**
...
3. Adja meg a tárfiók nevét. Ez lehet rövid nevét vagy teljes Tartománynevét.
...
5. Kattintson a jobb gombbal a hadoopservicelog tábla a **Navigator** ablaktáblán, és válassza **szerkesztése**. 4 oszlop megjelenik. Esetlegesen, törölje a **partíciókulcs**, **Sorkulcsa**, és **időbélyeg** oszlopok kiválasztva, majd **oszlopok eltávolítása** a a beállítások a szalagon.
...
Excel segítségével mostantól szűrési és rendezési szükség szerint. Természetesen érdemes lehet más oszlopokra (pl. üzenet) ahhoz, hogy a részletekbe menően tárhatják fel problémák jelentkeznek, de kiválasztásával, és a fent leírt oszlopok csoportosítási biztosít a Hadoop-szolgáltatás, mi történik egy decent képe. Az azonos ötlet a setuplog és hadoopinstalllog táblákra alkalmazhatók.
...
Az Azure-portálon kattintson egy HDInsight-fürt nevét, a fürt panel megnyitásához. A fürt paneljén kattintson **irányítópult**.

![Indítsa el a fürt-irányítópult](./media/hdinsight-debug-jobs/hdi-debug-launch-dashboard.png)

Amikor a rendszer kéri, adja meg a fürt rendszergazdai hitelesítő adatokat. Lekérdezés konzolon kattintson **Hadoop felhasználói felület**.
...
Az Azure-portálon kattintson egy HDInsight-fürt nevét, a fürt panel megnyitásához. A fürt paneljén kattintson **irányítópult**. Amikor a rendszer kéri, adja meg a fürt rendszergazdai hitelesítő adatokat. Lekérdezés konzolon kattintson **YARN felhasználói felületen**.

A YARN felhasználói felület segítségével tegye a következőket:

* **Fürt állapotának beolvasása**. A bal oldali ablaktáblán bontsa ki a **fürt**, és kattintson a **kapcsolatos**. A jelen fürt állapot adatait, például teljes kiosztott memória, magot használja, a fürt erőforrás-kezelő állapotának, a fürt verzió stb.
...
Azure-portálról kattintson az HDInsight hbase-ben a fürt nevét, a fürt panel megnyitásához. A fürt paneljén kattintson **irányítópult**. Amikor a rendszer kéri, adja meg a fürt rendszergazdai hitelesítő adatokat. Lekérdezés konzolon kattintson **HBase felhasználói felület**.

## <a name="hdinsight-error-codes"></a>HDInsight hibakódok
Az ebben a szakaszban részletezett hibaüzenetek biztosíthat a felhasználóknak az Azure HDInsight Hadoop megérteni, hogy képes a szolgáltatás az Azure PowerShell felügyeletekor lehetséges hibaállapotok, és tanácsokkal azokat a lépéseket, amelyek is találhatók. a hiba helyreállítása szükséges.
