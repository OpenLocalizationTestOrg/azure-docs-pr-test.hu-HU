...
* (Választható) **Visual Studio**: Azure SDK 2.5.1-es vagy újabb és a Data Lake Tools for Visual Studio. További információkért lásd: [Ismerkedés a Data Lake Tools for Visual Studio használatával](hdinsight-hadoop-visual-studio-tools-get-started.md).
...
1. Ha még nem telepítette a legújabb verzióját a Data Lake tools for Visual Studio, lásd: [Ismerkedés a Data Lake Tools for Visual Studio használatával](hdinsight-hadoop-visual-studio-tools-get-started.md).
...
2. Használja az alábbi parancsot példatopológia indításához:
    ...
    A paranccsal elindítja a példa WordCount-topológia a fürtön. Ez a topológia véletlenszerűen generálja a mondatok, és az összes szó a mondatok előfordulási száma.
...
A topológia a HDInsight alatt futó Storm által a a fürtön futó Nimbus szolgáltatással folytatott kommunikáció programozott módon telepítheti. [https://github.com/Azure-Samples/hdinsight-Java-Deploy-Storm-topology](https://github.com/Azure-Samples/hdinsight-java-deploy-storm-topology) példával Java-alkalmazás, amely bemutatja, hogyan kell telepíteni és elindítani a topológia a Nimbus szolgáltatáson keresztül.

## <a name="monitor-and-manage-visual-studio"></a>Megfigyelés és kezelés: Visual Studio

Ha egy topológia sikeresen elküldte a Visual Studio használatával a **Storm-topológiák** nézetére a fürt megjelenik-e. Válassza ki a topológiát a futó topológiát vonatkozó információk megtekintése a listából.
...
* **Az Ambari webes**: válasszon **szolgáltatások** a lap tetején, majd válassza ki **Storm**. Az a **összegzés** lapon jelölje be **Storm felhasználói felület Server**. A Storm felhasználói felület és a REST API-t futtató csomópont teljes Tartományneve van, az oldal tetején.
...
Információ a REST API visszaadott csak lehet a fürt vagy a virtuális gépek azonos Azure virtuális hálózaton a fürt belül használható. Például visszaadott Zookeeper-kiszolgáló teljesen minősített tartománynevét (FQDN) nem lesz elérhető az internetről.

## <a name="next-steps"></a>Következő lépések

Most, hogy megismerte segítségével telepítheti és figyelheti a topológia a Storm irányítópultjának használatával megtudhatja, hogyan lehet [Maven használatával fejlesztése Java-alapú topológiák](hdinsight-storm-develop-java-topology.md).
