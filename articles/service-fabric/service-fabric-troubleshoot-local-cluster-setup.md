...
Ha problémát tapasztal az Azure Service Fabric helyi fejlesztési fürtök való interakció során, tekintse át a következő javaslatok a lehetséges megoldásokról hello.
...
Hello DevClusterSetup parancsprogram futtatása során ehhez hasonló hibaüzenetet jelenik meg:
...
Zárja be hello aktuális PowerShell-ablakot, és nyissa meg rendszergazdaként egy új PowerShell-ablakot. Hello parancsprogrammal képes toosuccessfully kell.
...
#### <a name="problem"></a>Probléma
Ha megpróbál toorun bármelyik hello Service Fabric PowerShell-parancsmagok, például a `Connect-ServiceFabricCluster` egy Azure PowerShell-ablakban, ez nem sikerül, arról, hogy hello parancsmag nem ismerhető fel. hello ennek oka, hogy az Azure PowerShell hello 32 bites verzióját használja, a Windows PowerShell (még akkor is, 64 bites operációs rendszer verzió), mivel a Service Fabric parancsmagok hello csak 64 bites környezetben működik.

#### <a name="solution"></a>Megoldás
A Service Fabric parancsmagok futtatása mindig közvetlenül a Windows PowerShell.

> [!NOTE]
> Azure PowerShell legújabb verziójának hello nem hoz létre egy külön helyi, ez már nem jöjjön létre.
> 
> 

### <a name="type-initialization-exception"></a>Írja be az inicializálási kivétel
#### <a name="problem"></a>Probléma
Amikor PowerShell toohello fürthöz csatlakozik, System.Fabric.Common.AppTrace lásd hello hiba TypeInitializationException.
...
#### <a name="problem"></a>Probléma
A hívás tooConnect-ServiceFabricCluster meghiúsul, és ehhez hasonló hibát:
...
#### <a name="solution"></a>Megoldás
Zárja be hello aktuális PowerShell-ablakot, és nyissa meg rendszergazdaként egy új PowerShell-ablakot. Most tudnia kell csatlakozni toosuccessfully.
...
#### <a name="solution"></a>Megoldás
Ez a hiba általában akkor történik meg a szolgáltatás gazdafolyamatokon toostart manuálisan, ahelyett, hogy így hello Service Fabric futásidejű toostart azt meg.
...
> [!TIP]
> Ha a telepítés után a helyi fürt toobehave rendellenesen kezdődik, visszaállíthatja rajta a hello helyi fürt manager rendszer tálca alkalmazás segítségével. Ez eltávolítja a meglévő fürt hello, és állítson be egy új. Vegye figyelembe, hogy a telepített alkalmazások és a kapcsolódó adatokat távolítja el.
> 
> 
