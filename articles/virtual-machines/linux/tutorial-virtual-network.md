...
> * Csatlakoztassa a virtuális gépek alhálózathoz
> * Virtuális gép nyilvános IP-címeinek kezelése
> * Bejövő internet forgalmának biztonságossá tétele
> * Virtuális gép biztonságos a virtuális gépek forgalma
...
Nekünk a [az hálózati vnet létrehozása](/cli/azure/network/vnet#create) parancs futtatásával hozzon létre egy virtuális hálózatot. Ebben a példában a hálózati neve *mvVnet* , és egy címelőtagot *10.0.0.0/16*. Egy alhálózat is létrejön, melynek a neve *mySubnetFrontEnd* és a előtaggal *10.0.1.0/24*. Az oktatóanyag későbbi részében egy előtér-VM a alhálózathoz csatlakozik. 
...
Egy új alhálózatot hozzá lett adva a virtuális hálózat a [az alhálózaton virtuális hálózat létrehozása](/cli/azure/network/vnet/subnet#create) parancsot. Ebben a példában az alhálózat neve *mySubnetBackEnd* , és egy címelőtagot *10.0.2.0/24*. Ez az alhálózat összes háttérszolgáltatások használatos.
...
A nyilvános IP-cím lehetővé teszi, hogy az Azure-erőforrások elérhetővé az interneten. Ebben a szakaszban az oktatóanyag egy virtuális gép létrehozása bemutatásához nyilvános IP-címek használata.

### <a name="allocation-method"></a>Lefoglalási módszer

Egy nyilvános IP-címet, dinamikus vagy statikus oszthat ki. Alapértelmezés szerint dinamikusan lefoglalt nyilvános IP-cím. Dinamikus IP-címek van kiadva, ha egy virtuális gép felszabadítása. Ennek következtében az IP-cím módosítása bármely, amely tartalmazza a virtuális gép felszabadítás művelet során.

A kiosztási módszer állítható be a statikus, amely biztosítja, hogy az IP cím maradnak hozzárendelve a virtuális gép, egy megszüntetett állapot esetén. A statikusan lefoglalt IP-cím használata esetén nem adható meg az IP-címet saját magát. Ehelyett az történik a rendelkezésre álló címek készletét.

### <a name="dynamic-allocation"></a>Dinamikus lefoglalását

A virtuális gép létrehozásakor a [az virtuális gép létrehozása](/cli/azure/vm#create) parancs, az alapértelmezett nyilvános IP-cím címkiosztási módszerét dinamikus. A következő példában egy virtuális gép létrehozása egy dinamikus IP-címmel. 
...
Használatával egy virtuális gép létrehozásakor a [az virtuális gép létrehozása](/cli/azure/vm#create) paranccsal, tartalmazza a `--public-ip-address-allocation static` argumentum egy statikus nyilvános IP-címet hozzárendelni. Ez a művelet nem mutatja be ebben az oktatóanyagban azonban a következő szakaszban a dinamikusan kiosztott IP-címet egy statikusan lefoglalt címet változik. 
...
Gyakran egy virtuális Gépet nem kell elérhetőnek kell lennie az interneten keresztül. Egy virtuális gép, egy nyilvános IP-cím nélküli létrehozásához használja a `--public-ip-address ""` dupla idézőjelek között egy üres számú argumentum. Ez a konfiguráció az oktatóanyag későbbi részében bemutatott
...
Frissítse a meglévő nevű alhálózat *mySubnetBackEnd* rendelkező új NSG.
...
A háttér-virtuális gép jelenleg csak elérhető porton *22* és port *3306* az előtér-alhálózatból. A hálózati biztonsági csoport összes egyéb bejövő forgalom blokkolva van. Az NSG-szabályok konfigurációi megjelenítéséhez hasznos lehet. Térjen vissza a az NSG-konfiguráció a [az hálózati szabálylistában](/cli/azure/network/nsg/rule#list) parancsot. 
...
Ebben az oktatóanyagban létre és a védett virtuális gépek kapcsolatos Azure-hálózatok. Megtudta, hogyan, hogy:
