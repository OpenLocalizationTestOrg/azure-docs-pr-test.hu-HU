...
Hibaelhárítás megkezdése előtt győződjön meg arról, hogy a felhasználó és eszköz rendelkezik-e beállítva megfelelően, és, hogy az adott vállalati Állapothordozás követelményeinek teljesülnek-e az eszköz és a felhasználó által. 
...
2. Az eszköz csatlakoztatva, vagy tartományhoz és az Azure ad-vel regisztrált az Azure AD.
3. Az Azure Active Directory portálon **a vállalati Állapothordozás** engedélyezve van a könyvtárat a **konfigurálása** > **eszközök** > **felhasználók előfordulhat, hogy szinkronizálási beállítások és a vállalati alkalmazásadatok**. Minden felhasználó van kiválasztva, vagy a felhasználó szinkronizálásra keresztül a kiválasztott beállítás engedélyezve van, és a biztonsági csoport.
4. A felhasználó rendelkezik-e az Azure Active Directory Premium előfizetéssel hozzájuk rendelve.  
5. Az eszköz újraindítása után, és a vállalati Állapothordozás engedélyezése után a felhasználó bejelentkezése.

## <a name="information-to-include-when-you-need-help"></a>Ha segítségre van szüksége információk
Ha nem tudja megoldani a problémát az alábbi útmutatást, forduljon a Microsoft terméktámogatási szakemberek. Amikor kapcsolatba lép a őket, javasoljuk, hogy a következő információkat tartalmazza:
...
- **A felhasználó adatai** – milyen identitás az a felhasználó bejelentkezni az eszköz használatával? Hogyan a felhasználó bejelentkezik-e az eszközre? Vannak engedélyezett a szinkronizálás kijelölt biztonsági csoport egy részének? 
- **Az eszköz adatai** – az Azure AD csatlakoztatott eszköz, vagy a tartományhoz? Milyen build van az eszköz? Mik a legújabb frissítéseket?
...
1. Miután egy van konfigurálva ahhoz, hogy a vállalati Állapothordozás, bejelentkezési munkahelyi fiókját a tartományhoz való csatlakozás a Windows 10 rendszerű számítógépen. Ugrás a **beállítások** > **fiókok** > **a szinkronizálási beállítások** , és győződjön meg arról, hogy szinkronizálás és az egyéni beállításokat, és, hogy a beállítások oldal azt jelzi, hogy szinkronizálását végzi a munkahelyi fiókjával. Ellenőrizze, hogy ugyanazt a fiókot is használja a bejelentkezési fiókként **beállítások** > **fiókok** > **a információ**. 
2. Győződjön meg arról, hogy szinkronizálás azáltal, hogy néhány módosítást az eredeti számítógép, például a tálca áthelyezése a képernyő jobb oldali vagy felső oldalán a működik több számítógépen. Tekintse meg a változás propagálása a második géphez öt percen belül. 
 ...
 - Kell használnia ugyanazt a felhasználói fiókot a szinkronizáláshoz mindkét számítógépeken működéséhez –, a felhasználói fiókot, és nem a számítógépfiókot a vállalati Állapothordozás kötődik.

**Lehetséges probléma**: A beállítások lapon rendelkezik a be-és kikapcsolása szürkén jelenik meg, és nem egy fiókot, láthatja a szöveges "egyes Windows-szolgáltatások csak érhető el, ha egy Microsoft-fiók vagy a munkahelyi fiók használata". A probléma esetleg felmerülő legfeljebb beállított eszközök tartományhoz csatlakoztatott és a regisztrált az Azure AD, de az eszköz sikeresen nem hitelesítette az Azure ad Szolgáltatásba. Egy lehetséges oka, hogy az eszköz szabályzatot kell alkalmazni, de ez az alkalmazás aszinkron módon történik, és néhány órával késhet. A probléma ellenőrzéséhez lépéseit ellenőrizze, hogy ez az eszköz regisztrációs állapot ellenőrzéséhez hajtsa végre a helyzet.
