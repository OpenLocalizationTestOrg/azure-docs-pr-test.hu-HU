...
## <a name="how-can-i-roll-over-the-kerberos-decryption-key-of-the-azureadssoacct-computer-account"></a>Hogyan lehet I váltása Kerberos visszafejtési kulcsa a `AZUREADSSOACCT` számítógépfiók?

Fontos, hogy a Kerberos visszafejtési kulcsa gyakran váltása a `AZUREADSSOACCT` (amely az Azure AD) létrehozott számítógépfiók a helyszíni Active Directory-erdőben.
...
2. Hívás `Update-AzureADSSOForest -OnPremCredentials $creds`. Ez a parancs frissíti a Kerberos visszafejtési kulcs a `AZUREADSSOACCT` számítógép fiókját az adott Active Directory-erdőben, és frissíti az Azure AD-ben.
