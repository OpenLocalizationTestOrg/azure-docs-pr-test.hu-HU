...
Válassza ki **összes alkalmazás** konfigurált összes alkalmazások listájának megtekintéséhez. Az erőforrás panel az alkalmazáshoz, ahol jelentések megtekinthetők az alkalmazáshoz, és különböző beállítások kezelheti egy alkalmazás kiválasztása tölti be.
...
A **egyszeri bejelentkezés** panel kezdődik egy **mód** menü, amely lehetővé teszi, hogy az egyetlen bejelentkezés módban kell konfigurálni. Az elérhető lehetőségek a következők:

* **SAML-alapú bejelentkezés** – Ez a beállítás akkor érhető el, ha az alkalmazás támogatja a teljes összevont egyszeri bejelentkezés az Azure Active Directoryban a SAML 2.0 protokoll használatával.
...
* **A bejelentkezési kapcsolódó** – korábbi nevén a "Meglévő egyszeri bejelentkezéshez", ez a beállítás lehetővé teszi a rendszergazdák számára, hogy a felhasználó Azure AD hozzáférési Panel vagy az Office 365 alkalmazásindító helyezze el az alkalmazás mutató hivatkozást.
...
A **SAML-alapú bejelentkezés** lehetőséget választja, megnyílik egy panel, amelyen a forgatókönyv négy részből áll:
...
* **URL-cím bejelentkezési** – Ha a felhasználó megnyitja a jelentkezik be az alkalmazás számára. Ha az alkalmazás végre szolgáltatás szolgáltató által kezdeményezett egyszeri bejelentkezéshez, majd amikor a felhasználók megnyitják az URL-cím van konfigurálva, a szolgáltató az Azure AD-hitelesítéséhez, és a felhasználói bejelentkezés szükséges átirányítást végzi. Ha ez a mező nem üres, majd az Azure AD fogja használni az Office 365 és az Azure AD hozzáférési Panel az alkalmazás elindításához az URL-cím. Ha kihagyja, akkor az Azure AD helyette végez identitásszolgáltató – amikor az alkalmazás elindul az Office 365, az Azure AD hozzáférési Panel vagy az Azure ad-bejelentkezés kezdeményezett egyszeri bejelentkezési URL-cím.
* **Azonosító** -ez URI egyedileg kell azonosítania az alkalmazást, a mely egyszeri bejelentkezés konfigurálása történik. Ez az érték, amely az Azure AD küld vissza az SAML-jogkivonatra alkalmazásának a célközönség paraméterként, és az alkalmazás várhatóan ellenőrzéséhez. Ez az érték is az entitás azonosítója a SAML metaadatai között, az alkalmazás által biztosított jelenik meg.
