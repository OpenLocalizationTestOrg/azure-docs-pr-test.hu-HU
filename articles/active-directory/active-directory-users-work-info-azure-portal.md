---
title: "az Azure Active Directoryban egy felhasználó aaaAdd, vagy módosítsa a munkahelyi adatait |} Microsoft Docs"
description: "Bemutatja, hogy tooadd telefonszámokat, a részleg neve és egyéb működik egy felhasználó adatait az Azure Active Directoryban"
services: active-directory
documentationcenter: 
author: curtand
manager: femila
editor: 
ms.assetid: a6f16152-53f4-491e-a1c3-157f41bc39fa
ms.service: active-directory
ms.workload: identity
ms.tgt_pltfrm: na
ms.devlang: na
ms.topic: article
ms.date: 06/27/2017
ms.author: curtand;jeffsta
ms.reviewer: jeffsta
ms.openlocfilehash: 53e645b2017cefd9ebf6d724526f462122739950
ms.sourcegitcommit: 523283cc1b3c37c428e77850964dc1c33742c5f0
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 10/06/2017
---
# <a name="add-or-change-work-information-for-a-user-in-azure-active-directory"></a>Az Azure Active Directoryban egy felhasználó a munkahelyi adatok hozzáadása vagy módosítása
Ez a cikk azt ismerteti, hogyan tooadd, vagy módosítsa a munkahelyi adatok például telefonszámok vagy a részleg neve, az Azure Active Directory (Azure AD) felhasználó számára. A szervezetben új felhasználók hozzáadásával kapcsolatos további információkért lásd: [adja hozzá az új felhasználók tooAzure Active Directory](active-directory-users-create-external-azure-portal.md).

## <a name="toochange-work-information"></a>toochange munkahelyi adatokat
1. Jelentkezzen be toohello [Azure-portálon](https://portal.azure.com) egy olyan fiókkal, amely hello címtár globális rendszergazdája.
2. Válassza ki **további szolgáltatások**, adja meg **felhasználók és csoportok** hello szövegmezőbe, és válassza ki **Enter**.

   ![Nyitó felhasználók kezelése](./media/active-directory-users-work-info-azure-portal/create-users-user-management.png)
3. A hello **felhasználók és csoportok** panelen válassza **felhasználók**.

   ![Nyitó hello felhasználók panel](./media/active-directory-users-work-info-azure-portal/create-users-open-users-blade.png)
4. A hello **felhasználók és csoportok - felhasználók** panelen válassza ki a megfelelő felhasználói hello listából.
5. A kiválasztott felhasználó hello hello panelen válassza ki a **munkahelyi adatai**.

    ![Nyitó munkahelyi adatokat](./media/active-directory-users-work-info-azure-portal/active-directory-create-users-work-info.png)
6. Adatok hozzáadása vagy módosítása hello munka. Ezt követően hello parancssávon válassza **mentése**.

## <a name="next-steps"></a>Következő lépések
* [Adja hozzá az új felhasználók tooAzure Active Directory](active-directory-users-create-azure-portal.md)
* [A felhasználó az Azure Active Directoryban hello jelszó visszaállítása](active-directory-users-reset-password-azure-portal.md)
* [A felhasználó az Azure Active Directoryban tooadministrator szerepkörök hozzárendelése](active-directory-users-assign-role-azure-portal.md)
* [Adja hozzá, vagy egy Azure Active Directoryban lévő felhasználói profil adatainak módosítása](active-directory-users-profile-azure-portal.md)
* [A felhasználó töröl egy könyvtárat az Azure Active Directoryban](active-directory-users-delete-user-azure-portal.md)
