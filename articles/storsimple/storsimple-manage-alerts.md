...
---
# <a name="use-hello-storsimple-manager-service-tooview-and-manage-storsimple-alerts"></a>Hello StorSimple Manager szolgáltatás tooview használja, és a StorSimple-riasztások kezelése
## <a name="overview"></a>Áttekintés
Hello **riasztások** hello StorSimple Manager szolgáltatás lapján lehetőséget biztosít az Ön tooreview és törölje a jelet a StorSimple eszköz – kapcsolatos riasztások valós idejű alapon. Ezen a lapon központilag figyelheti hello ügynökállapottal kapcsolatos hibákkal StorSimple eszközt, és hello általános Microsoft Azure StorSimple megoldáshoz.
...
1. Nyissa meg túl**eszközök** > **konfigurálása** hello eszközhöz.
2. A **riasztási beállítások**, állítsa be a következőket hello:
...
3. egy teszt e-mail-értesítések toosend választógombot hello nyíl ikonra túl**teszt e-mail küldése**. hello tesztértesítés továbbítja a StorSimple Manager szolgáltatás hello jelennek-állapotüzenetek. 
4. Ha hello következő üzenet jelenik meg, kattintson **OK**. 
   
    ![Riasztások által küldött értesítő e-mail tesztelése](./media/storsimple-manage-alerts/HCS_AlertNotificationConfig3.png)
   
   > [!NOTE]
   > Hello értesítési nem lehet tesztüzenetet küldeni, ha a StorSimple Manager szolgáltatás hello egy megfelelő üzenetet jelenít meg. Kattintson a **OK**, várjon néhány percet, és próbálkozzon toosend az értesítési tesztüzenet újra. 
   > 
   > 

## <a name="view-and-track-alerts"></a>Megtekintheti, és nyomon követéséhez értesítések
hello StorSimple Manager szolgáltatás irányítópultját tesz lehetővé az eszközök, súlyossági szint szerint a riasztások számának hello kiadványok.

![Riasztások irányítópult](./media/storsimple-manage-alerts/admin_alerts_dashboard.png)

Hello súlyossági szint kattintva megnyílik a hello **riasztások** külön-külön hello eredmények tartalmazzák a csak az olyan hello riasztásokat, amelyek megfelelnek az adott súlyossági szintet.
...
Szükség lehet arra riasztások hatékonyabb toorun jelentéseket, hogy tekintse át, és törölje őket a csoportokban. Emellett hello **riasztások** lapon too250 figyelmeztetéseket tudja megjeleníteni. Túllépte a riasztások számát is, ha nem az összes riasztás hello alapértelmezett nézetben jelenik meg. A következő mezők toocustomize mely riasztások megjelenítésének hello kombinálva:
...
| Közelgő meghajtó hibát észlelt. | Közelgő meghajtó hibát észlelt. |Azt észleltük közelgő meghajtó hibát hello hardver összetevő "tárolóhelyen lévő meghajtó <*tárolóhely-azonosító*>, ház <*ház azonosító*>". Vegye figyelembe, hogy a meghajtó helyett. <br> Hello lemezcsere megkezdése előtt tekintse át a következő információ hello.<br><br>Ha az eszközön egynél több meghibásodott lemezt, távolítsa el az egynél több SSD és HDD bármikor. Így adatvesztést eredményezhet.<br><br>Ellenőrizze, hogy egy helyettesítő SSD helyez egy SSD korábban tartalmazó tárhely. hello esetében is igaz egy HDD.<br><br>Üzembe helyezési ponti számozása 0 too11-től. Egy meghibásodott lemez tárolóhelye 2 leképezi a hibás lemezen tooa tárolóhelye 3 hello eszköz (a bal felső hello).<br><br>Lemezcsere kapcsolatos további információkért lépjen toohttps://go.microsoft.com/fwlink/?linkid=838653. Ha a probléma továbbra is fennáll, forduljon a Microsoft támogatási https://go.microsoft.com/fwlink/?linkid=838654 keresztül. |
