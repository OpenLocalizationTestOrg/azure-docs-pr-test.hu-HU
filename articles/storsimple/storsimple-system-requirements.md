...
| 9354 TCP |Kimenő |WAN |Igen |hello StorSimple eszköz toocommunicate a StorSimple Manager szolgáltatás hello hello kimenő portot használja. |
...
| --- | --- | --- |
| `https://*.storsimple.windowsazure.com/*`<br>`https://*.accesscontrol.windows.net/*`<br>`https://*.servicebus.windows.net/*` |StorSimple Manager szolgáltatás<br>Access Control Service<br>Azure Service Bus |A felhőalapú hálózati illesztők |
...
| Az URL-minta | Összetevő/funkció | Eszköz IP-címek |
| --- | --- | --- |
| `https://*.storsimple.windowsazure.us/*`<br>`https://*.accesscontrol.usgovcloudapi.net/*`<br>`https://*.servicebus.usgovcloudapi.net/*` |StorSimple Manager szolgáltatás<br>Access Control Service<br>Azure Service Bus |A felhőalapú hálózati illesztők |
...
útválasztási metrika algoritmusok hello a StorSimple eszközön futó hello szoftver verziójától függően eltérőek.

**Feloldja a korábbi tooUpdate 1**

Ez magában foglalja a szoftver verziója előzetes tooUpdate hello GA például 1, 0,1, 0,2 vagy 0,3 kiadás. útválasztási metrika hello sorrendben a következőképpen történik:
...
**1. frissítés és a korábbi tooUpdate 2-től kezdődő kiadások**

Ez magában foglalja a szoftver verziójával, például 1, 1.1-es vagy 1.2-es. útválasztási metrika hello sorrendben az határozza meg az alábbiak szerint:
   ...
   *DATA 0 > legutóbbi konfigurált 10 GbE hálózati adapter > egyéb 10 GbE hálózati adapter > legutóbbi konfigurált 1 GbE hálózati adapter > egyéb 1 GbE hálózati adapter*
   ...
   1. frissítést, a DATA 0 hello útválasztási metrikája legyen hello legalacsonyabb; ezért az összes hello felhőalapú forgalom DATA 0 keresztül történik. Jegyezze fel a ez Ha egynél több felhő-kompatibilis hálózati adaptert a StorSimple eszköz.

**Frissítés 2-től kezdődő kiadások**

Frissítés 2 a hálózattal kapcsolatos fejlesztéseket tartalmaz, és hello útválasztási metrika megváltozott. hello viselkedése lehet az alábbiak.
...
* hello amelyben hello felhőforgalom hello hálózati illesztők keresztül továbbítódik sorrendje:
    ...
    *ahol a hello számok zárójelben hello megfelelő útválasztási metrika jelzi.*
...
* Is riasztás esetén a StorSimple eszköz VIP hiba esetén. További információ: túl[riasztás rövid összefoglaló](storsimple-manage-alerts.md).
...
* Különítse el hello iSCSI és a felhőalapú forgalom dedikált hálózati adapterek iSCSI és a felhőalapú hozzáférés az eszközön. További információkért lásd: hogyan túl[módosítása a hálózati adapterek](storsimple-modify-device-config.md#modify-network-interfaces) a StorSimple eszköz.
...
* Csere, közben figyelése hello **hardverállapot** a hello **karbantartási** hello hello SSD és HDD meghajtókon a lapon. Zöld pipa állapot azt jelzi, hogy hello lemezek kifogástalan vagy OK, mivel egy piros felkiáltójel jelzi egy sikertelen SSD és HDD.
...
* Egy EBOD vezérlő modul csere közben folyamatosan hello állapotának figyelése a StorSimple Manager szolgáltatás hello hello összetevő elérésével **karbantartási** > **hardverállapot**.
...
* [Megtudhatja, hogyan toodeploy a StorSimple megoldásban](storsimple-deployment-walkthrough-u2.md).
