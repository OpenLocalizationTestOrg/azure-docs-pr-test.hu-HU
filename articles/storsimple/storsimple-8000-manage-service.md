...
A StorSimple 8000 series mostantól kezelhető az Azure portálon. Ha rendelkezik meglévő szolgáltatással a StorSimple eszközök kezelésére, javasoljuk, hogy a szolgáltatás helyezi át az Azure-portálon. A StorSimple Manager szolgáltatás a klasszikus Azure portálon 2017. szeptember 30 után nem érhető el.

Az Azure Portalra való migrálás lehetősége szakaszokban érhető el. Ha az Azure Portalra való migrálás lehetősége nem érhető el, de Ön szeretné végrehajtani, és már mérlegelte a potenciális hatásokat a [Szempontok a váltáshoz](#considerations-for-transition) című dokumentum alapján, [elküldhet egy kérést](https://aka.ms/ss8000-cx-signup) erről.
...
* Az eszköz fut frissítés 3.0-s vagy újabb. Ha az eszköz egy régebbi verziója fut, telepítse a legújabb frissítéseket. További információkért látogasson el [Update 4 telepítése](storsimple-8000-install-update-4.md). A StorSimple felhő készülék (8010-es/8020-as modell) használata esetén hozzon létre egy új felhőalapú készülék frissítés 4.0-s verzióját. 
...
* A meglévő Azure Service Management (ASM) PowerShell-parancsmagok használata nem támogatott. Frissítse a parancsfájlok az Azure Resource Manageren keresztül eszközei kezeléséhez.
...
1. Ugrás a meglévő StorSimple Manager szolgáltatás a klasszikus portálon.

2. Megjelenik egy értesítés, amely tájékoztatja, hogy a StorSimple Device Manager szolgáltatás jelenleg elérhető az Azure portálon. Vegye figyelembe, hogy az Azure portálon, a szolgáltatás nevezzük StorSimple Device Manager szolgáltatást.
...
Az átmenet befejezése után az eszközök a StorSimple Device Manager szolgáltatás az Azure portálon keresztül is kezelheti.

Az Azure portálon csak a 3.0-s és újabb rendszer Update futtatása StorSimple-eszközöket támogatja. A régebbi verzióját futtató eszközök csak korlátozottan támogatják. A következő tábla summrizes, hogy mely műveletek támogatottak versios frissítés 3.0 előtt fut, miután áttelepítette klasszikus Azure portálra az eszközön.
