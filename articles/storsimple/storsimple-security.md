...
* **A Microsoft Azure-ban tárolt StorSimple Manager szolgáltatás** – a felügyeleti szolgáltatás, amellyel konfigurálhatja, és helyezze üzembe a StorSimple eszköz.
...
A StorSimple Manager szolgáltatás egy olyan felügyeleti szolgáltatás a Microsoft Azure-ban üzemeltetett, és minden StorSimple eszközök felügyeletére, amelyek a szervezet közvetített használt. A StorSimple Manager szolgáltatás által a szervezeti hitelesítő adataival bejelentkezni a klasszikus Azure-portálon webböngészőn keresztül végezheti el. 

A StorSimple Manager szolgáltatás használatához, hogy a szervezet rendelkezik-e az Azure-előfizetéssel, amely tartalmazza a StorSimple. Az előfizetés szabályozza a klasszikus Azure portálon keresztül elérhető funkciókat. Ha a szervezet nem rendelkezik Azure-előfizetés és a velük kapcsolatos tudnivalókért lásd: szeretné [regisztráció az Azure-bA szervezetként](../active-directory/sign-up-organization.md). 

Mivel a StorSimple Manager szolgáltatás az Azure-ban üzemel védi az Azure biztonsági funkciók. A Microsoft Azure által biztosított biztonsági funkciókkal kapcsolatos további információkért látogasson el a [Microsoft Azure Trust Center](https://azure.microsoft.com/support/trust-center/security/).
...
Kizárólag engedélyezett StorSimple eszközök engedélyezettek a StorSimple Manager szolgáltatás az Azure-előfizetéshez létrehozott csatlakozni. Egy eszköz hitelesítése, regisztrálnia kell azt a StorSimple Manager szolgáltatásban Szolgáltatásregisztrációs kulcs megadásával. A szolgáltatás regisztrációs kulcsának egy véletlenszerű kulcsot 128 bites, a klasszikus Azure portálon jön létre. 

![Szolgáltatásregisztrációs kulcs](./media/storsimple-security/ServiceRegistrationKey.png)

Megtudhatja, hogyan be a szolgáltatás regisztrációs kulcsának, keresse fel [2. lépés: Szolgáltatásregisztrációs kulcs lekérése](storsimple-deployment-walkthrough.md#step-2-get-the-service-registration-key).

A szolgáltatás regisztrációs kulcsának, mint egy hosszú kulcs 100 + karaktereket tartalmaz. Másolja a vágólapra a kulcsot, és mentse egy biztonságos helyre a fájlt, hogy szükség esetén további eszközök engedélyezésére használja. Ha a szolgáltatás regisztrációs kulcsának elveszik, az első eszköz regisztrálása után, a StorSimple Manager szolgáltatás hozhat létre egy új kulcsot. Ez nem érinti a meglévő eszközök működését. 
...
* Távfelügyelet alapértelmezés szerint ki van kapcsolva. A StorSimple Manager szolgáltatás segítségével engedélyezheti azt. Biztonsági szempontból ajánlott távelérés csak időszakban a ténylegesen igényelt engedélyezni kell.
...
A CHAP és a StorSimple megoldás konfigurálásának kapcsolatos további információkért látogasson el [CHAP konfigurálása a StorSimple eszköz](storsimple-configure-chap.md).
...
A StorSimple Manager szolgáltatás elsődleges célja, kezelése és konfigurálása a StorSimple eszközt. A StorSimple Manager szolgáltatás fut a Microsoft Azure-ban. A klasszikus Azure portál használatával adja meg az eszköz konfigurációs adatokat, és a Microsoft Azure segítségével a StorSimple Manager szolgáltatás elküldi az adatokat az eszközre. StorSimple a rendszer, aszimmetrikus kulcspárokat használatával biztosítható, hogy a biztonsági sérülését jelenti az Azure-szolgáltatás nem lesz tárolt információ-biztonság sérülését. 
...
3. A tanúsítvány nyilvános kulcsát biztonságosan szeretné elérhetővé tenni a StorSimple Manager szolgáltatáshoz, és a titkos kulcs marad az eszközön.
...
* Amikor a felhőalapú tárolás titkosítási kulcsát a StorSimple Manager szolgáltatásban, a kulcs titkosított a szolgáltatásadat-titkosítási kulcs nyilvános részének használatával, és elküldheti az eszközre.
...
* Rendszeres időközönként az Azure rendszergazdai kell módosítani, vagy az elsődleges vagy másodlagos kulcs újragenerálása közvetlenül a tárfiók eléréséhez a tárolási szakaszában a klasszikus Azure portál használatával.
...
| --- | --- | --- |
| RSA |2048 |RSA 1-PKCS 1.5-ös verzióját használják a klasszikus Azure portálra az eszközre küldött konfigurációs adatok titkosítása: például a tárolási fiók hitelesítő adatait, a StorSimple eszköz konfigurációs, és a felhőalapú tárolás titkosítási kulcsokat. |
| AES |256 |A CBC AES segítségével titkosíthatja a szolgáltatásadat-titkosítási kulcs nyilvános részének a klasszikus Azure portálra a StorSimple eszközön történő továbbítás előtt. Azt is használják a StorSimple eszköz adattitkosítás az adatok a felhőalapú társzolgáltatás fiókja történő elküldése előtt. |
...
**V:** ezt a kulcsot jött létre, amikor a StorSimple Manager szolgáltatás létrehozásakor. A StorSimple Manager szolgáltatás használatával csatlakozzon az eszközhöz, használhatja a szolgáltatás gyors üzembe helyezési oldal megtekintéséhez, vagy a szolgáltatás regisztrációs kulcsának újragenerálása. Egy új szolgáltatás regisztrációs kulcs nem érinti a meglévő regisztrált eszközöket. Útmutatásért tekintse:

* [Megtekintéséhez vagy a szolgáltatás regisztrációs kulcsának újragenerálása](storsimple-service-dashboard.md#view-or-regenerate-the-service-registration-key)
...
* [A StorSimple Manager szolgáltatással a storsimple-jelszavak módosítása](storsimple-change-passwords.md)
* [A szolgáltatásadat-titkosítási kulcs módosítása](storsimple-service-dashboard.md#change-the-service-data-encryption-key)
...
**V:** töltse le az új tanúsítványt a StorSimple Manager szolgáltatás, és adja meg azt a tanúsítványtárolójába a távoli ügyfelek telepíteni kell. Útmutatásért tekintse:

* [Tanúsítvány importálása parancsmag](https://technet.microsoft.com/library/hh848630.aspx)

**K:** az adatok védelme, ha a StorSimple Manager szolgáltatás sérült?

**V:** szolgáltatás konfigurációs mindig adattitkosítás a nyilvános kulccsal webböngészőben megtekintésekor. A szolgáltatás nem fér hozzá a titkos kulcsot, mert a szolgáltatás nem lesz képes jelennek meg az adatok. Ha a StorSimple Manager szolgáltatás biztonsága sérül, akkor ennek nincs hatása, mivel nincsenek a StorSimple Manager szolgáltatásban tárolt kulcsok.
