...
Ebben az eljárásban először létre kell hoznia egy event hub névtér, és ezután hozzáadhat egy eseményközpontot, hogy namepsace. Event hub névterek logikailag csoportosítják a kapcsolódó esemény bus példányok szolgálnak. 
...
Mielőtt elkezdené a TelcoGenerator alkalmazást, akkor konfigurálja úgy, hogy hívás rekordok küld az imént létrehozott eseményközpont.
...
1. Az irányítópult vagy a **összes erőforrás** panelen, megkeresheti, és válassza ki a `sa_frauddetection_job_demo` Stream Analytics-feladat. 
...
3. Kattintson a  **+ &nbsp;Hozzáadás** , és ezután adja meg a panel ezekkel az értékekkel:
...
Egy nagyon egyszerű lekérdezést egyszerűen előfordulhat, hogy a bejövő adatokat olvasni. Azonban Ön gyakran lekérdezéseket hozhat létre keresse meg a meghatározott adatok vagy az adatok kapcsolatok. Az oktatóanyag ezen részében létrehoz és további elemzés céljából egy bemeneti adatfolyam alakíthatja át néhány módszer több lekérdezések tesztelése. 
...
2. A portálon térjen vissza a Streaming Analytics-feladat panelen. (Ha korábban bezárta a panelen, keresse meg a `sa_frauddetection_job_demo` a a **összes erőforrás** panel.)
...
4. Az a **lekérdezés** panelen kattintson a Tovább gombra a pont a `CallStream` adja meg, és válassza ki **az adatokat a bemeneti**.
...
1. Az Azure portálon térjen vissza a Streaming Analytics-feladat panelen. (Ha korábban bezárta a panelen, keresse meg a `sa_frauddetection_job_demo` a a **összes erőforrás** panel.)
2. Az a **feladat topológia** területen kattintson a **kimeneti** mezőbe. 
3. Az a **kimenetek** panelen kattintson  **+ &nbsp;Hozzáadás** , és ezután adja meg a panel ezekkel az értékekkel:
...
3. Az a **indítási feladat** panelen, a feladat kimenete kezdési időpontja, jelölje be a **most**. 
...
1. A Streaming Analytics-feladat leállítása. Az a **feladatok** panelen kattintson a **leállítása** tetején.
...
Ez az oktatóanyag a következő cikkek továbbra is:
...
* [Azure Stream Analytics adatok tárolása az az Azure Redis Cache használata az Azure Functions](stream-analytics-functions-redis.md). Ez a cikk bemutatja, hogyan használja az Azure Functions az Azure Redis cache-keresztül a Service Bus-üzenetsorba csalárd hívások.
