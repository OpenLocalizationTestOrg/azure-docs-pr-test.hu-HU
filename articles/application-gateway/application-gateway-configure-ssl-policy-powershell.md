...
Megtudhatja, hogyan tooconfigure SSL házirend verziója és az Application Gateway-titkosítócsomagjai. Választhat egy [előre meghatározott házirendek](#predefined-ssl-policies) , amely tartalmazza az SSL-házirend verziók különböző konfigurációt, és engedélyezve van a titkosító csomagok. Akkor is hello képességét toodefine egy [egyéni SSL-házirend](#configure-a-custom-ssl-policy) a követelmények alapján.

## <a name="get-available-ssl-options"></a>Elérhető az SSL-beállítások beolvasása

Hello `Get-AzureRMApplicationGatewayAvailableSslOptions` parancsmag felsorolja elérhető előre meghatározott házirendek, a rendelkezésre álló titkosító csomagok és a protokollverziója konfigurálható. hello alábbi példában látható egy példa kimenet hello parancsmagot.
...
Alkalmazásátjáró 3 előre meghatározott házirendek használható tartalmaz. Hello `Get-AzureRmApplicationGatewaySslPredefinedPolicy` parancsmag beolvassa a házirendek. Minden egyes házirend rendelkezik különböző protokollverziója és titkosító csomagok engedélyezve van. Ezek előre meghatározott házirendek tooquickly egy SSL-házirend konfigurálása az Alkalmazásátjáró. Alapértelmezés szerint **AppGwSslPolicy20170401** van kiválasztva, ha nem adott SSL-házirend lett meghatározva.

hello következő az való futtatásának `Get-AzureRmApplicationGatewaySslPredefinedPolicy`.
...
a következő példa hello Alkalmazásátjáró egyéni SSL-házirend beállítása. Állítja a minimális protokollverzió hello túl`TLSv1_1` és lehetővé teszi, hogy hello következő titkosító csomagok:

* TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384
* TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256

> [!IMPORTANT]
> Legalább egy titkosítási csomagok a következő lista hello meg kell adni, ha egy egyéni SSL-házirend konfigurálása. Alkalmazásátjáró RSA SHA256 titkosító csomagok háttérbeli felügyeleti használ.
> * TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256 
> * TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256
> * TLS_DHE_RSA_WITH_AES_128_GCM_SHA256
> * TLS_RSA_WITH_AES_128_GCM_SHA256
...
hello alábbi példa hoz létre egy új Alkalmazásátjáró egy előre definiált SSL-házirend.
...
Látogasson el [Alkalmazásátjáró átirányítási áttekintése](application-gateway-redirect-overview.md) toolearn hogyan forgalmat tooredirect HTTP, HTTPS-végpont tooa.
